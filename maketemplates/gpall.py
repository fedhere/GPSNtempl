...
def gpit(thissn):
     ...
     if UNPICKLE:
          print "\n\n\n", here
          PICKLE = False
          print ('gplcvs/'+sn+"_gp_ebmv%.2f.pkl"%myebmv,
                 'gplcvs/'+sn+"_gplcv_ebmv%.2f.pkl"%myebmv)
          if os.path.isfile('gplcvs/'+sn+"_gp_ebmv%.2f.pkl"%myebmv) \
             and os.path.isfile('gplcvs/'+sn+"_gplcv_ebmv%.2f.pkl"%myebmv):
               thissn.gp =  pkl.load(open('gplcvs/'+sn+"_gp_ebmv%.2f.pkl"%myebmv, "rb"))
               
               gpmag = pkl.load(open('gplcvs/'+sn+"_gplcv_ebmv%.2f.pkl"%myebmv, "rb"))
               if PLOT:
                    print ("hereherehere")
                    for b in thissn.filters:
                         if thissn.filters[b] > 1:
                              ...
                              #           bbox_inches='tight')
                              if PRETTY:
                                   print ("gpplots/"+thissn.name+"_"+b+".gp.pdf")
                              else:
                                    print ("gpplots/"+thissn.name+"_"+b+".gp.png")                                  
          else:
               PICKLE = True
     ...
     if PICKLE:
          ...
          for b in thissn.filters:
               ...
               if thissn.filters[b] > 1:
                 ...
                 #     pass#
                 #pl.show()
          pkl.dump(gpmag, open('gplcvs/' + sn + \
                               "_gplcv_ebmv0.00f.pkl", "wb"))
          pkl.dump(thissn.gp, open('gplcvs/' + sn + \
         ...
         if np.isnan(thissn.Vmax) or thissn.Vmax == 0:
              ...
              if '06gi' in thissn.snnameshort:
                   try:
                        ...
                        thissn.gp = pkl.load(open('gplcvs/' + sn + \
                                                  "_gp_ebmv0.00.pkl", "rb"))
