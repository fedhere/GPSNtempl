...
def type_divider(sourcedata):
    ...
    return ss_IIb, ss_Ib, ss_Ic, ss_Ic_bl, ss_others
...
def bokehplot(source, b):
    ...
    ss_IIb, ss_Ib, ss_Ic, ss_Ic_bl, ss_others = type_divider(source)
        
    for i, tp in enumerate(['Ib', 'IIb', 'Ic','Ic_bl', 'others']):
        ...
        globals()['p%s%s' % (i,b)] = bokehfigure(plot_width=600, plot_height=300,
                    tools=globals()['TOOLS%s%s' % (i,b)], title="SNe lightcurves of type %s in band %s"%(tp, b))
...
#         update_xyaxis(globals()['p%s%s' % (i,b)], (miny - 0.5, maxy + 0.5))
        ...
        # layout1 = column(globals()['p%s%s' % (i,b)])
        # bokehsave(layout1)

    # layout2 = column(globals()['p0%s' % (b)], globals()['p1%s' % (b)], globals()['p2%s' % (b)],\
    #                    globals()['p3%s' % (b)], globals()['p4%s' % (b)])
    # bokehsave(layout2)

            
    

        # print ("\n\n bokeh plot saved to " + "outputs2/UberTemplate_%s.html"%\
        #           (b + 'p' if b in ['u', 'r', 'i'] else b))
        #curdoc().add_root(layout)
        #bokehshow(layout)    
    return (globals()['p0%s' % (b)], globals()['p1%s' % (b)], globals()['p2%s' % (b)], globals()['p3%s' % (b)], globals()['p4%s' % (b)])    
...
def doall(b = su.bands, weights_plot = False, weights_heatmap = False):
    ...
    for b in workBands:
        ...
        for i, tmp in enumerate(data['phase']):
            ...
            if len(tmp)<4:
                less_than_three_dp[0].append(b)
                less_than_three_dp[1].append(data['name'][i])
                less_than_three_dp[2].append(len(tmp))
    ...
    #             if verbose: print (np.abs(tmp[magoffset]).min())
        ...
        globals()['p0%s' % (b)], globals()['p1%s' % (b)], globals()['p2%s' % (b)], globals()['p3%s' % (b)], globals()['p4%s' % (b)] = bokehplot(sourcedata, b)


    htmlout = "outputs2/UberTemplate_PerType_PerBand2.html"
    output_file(htmlout)
    ...
    layout = gridplot(bkpl, plot_width=500, plot_height=500, toolbar_location='right')
