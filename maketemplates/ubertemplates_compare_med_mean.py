...
def preplcvs(inputSNe, workBands):
    ...
    for f in inputSNe:
        ...
        for b in workBands:
            ...
            # try:
            #     temp_mag = thissn.photometry[b]['mag'][indx][(thissn.photometry[b]['phase'][indx]>-10) & (thissn.photometry[b]['phase'][indx]<10)]
            #     temp_phs = thissn.photometry[b]['phase'][indx][(thissn.photometry[b]['phase'][indx]>-10) & (thissn.photometry[b]['phase'][indx]<10)]
            #     phs_peak = temp_phs[np.argmin(temp_mag)]
            # except:
            #     phs_peak = 0
            if len(thissn.photometry[b]['mjd'][indx]) != 0:
                xmin = thissn.photometry[b]['mjd'][indx].min()

                if xmin - thissn.Vmax < -1000:
                    x = thissn.photometry[b]['mjd'][indx] - thissn.Vmax + 2400000.5

                elif xmin - thissn.Vmax > 1000:
                    x = thissn.photometry[b]['mjd'][indx] - thissn.Vmax - 2400000.5
                else:
                    x = thissn.photometry[b]['mjd'][indx] - thissn.Vmax
            else:
                x = thissn.photometry[b]['mjd'][indx]
