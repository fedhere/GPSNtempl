...
fig, axs = plt.subplots(1, 3, figsize=(17*3, 15),
...
for k in range(1):
    ...
    for i, b in enumerate(bands):    
        ...
        for j in range(len(atypicals_phot[b]['x'])):
            ...
            axs[i].errorbar(atypicals_phot[b]['x'][j], atypicals_phot[b]['y'][j],
        ...
        axs[i].plot(tmpl[b][SNTYPE]['t'], tmpl[b][SNTYPE]['rollingMedian'],
                              color='k', linewidth=6)
        axs[i].fill_between(tmpl[b][SNTYPE]['t'], tmpl[b][SNTYPE]['rollingPc75'], tmpl[b][SNTYPE]['rollingPc25'],
                                      color='grey', alpha=0.5)
        axs[i].set_xlim(-25, 105)
        axs[i].set_ylim(-4.5, .5)
        if k == 3:
            axs[i].set_ylim(-4.5, 1)
        axs[i].text(0.8, 0.9, b + ', ' + SNTYPE, transform=axs[i].transAxes, size=60)
...
plt.savefig(
    os.getenv("SESNPATH") + 'maketemplates/outputs/output_plots/all_Anna_Ho_IIb.pdf',
    bbox_inches='tight')
